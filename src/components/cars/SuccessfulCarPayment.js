    ...
    return (
        ...
        <div className="payment-success-card">
            ...
            <p className="payment-success-message">Your payment for <strong>{location.state.car.model}</strong> for <strong>${location.state.totalPrice}</strong> has been made successfully!
                <br/> Pick-up date: <strong>{moment(location.state.dates.startDate).subtract(1, 'months').format("DD-MM-YY")}</strong> / Drop off date: <strong>{moment(location.state.dates.endDate).subtract(1, 'months').format("DD-MM-YY")}</strong>.
